...
class TraitementConnexion extends TraitementCommun{
	...
	function connecter(){
		$this->lireChamps('Classtraitement');
		if($this->tabInfos['Classtraitement'] == 'Inscription'){
			new TraitementInscription();
		}
		else if($this->tabInfos['Classtraitement'] == 'Login'){
			...
			if ($this->emailExists()){
				...
				if($this
					...
					$objSession->set("login", $login);
					$objSession->set("level", $level);
